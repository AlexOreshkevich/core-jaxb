package by.prognoz.exjaxb;

import by.prognoz.exjaxb.builder.MarshallerBuilder;
import by.prognoz.exjaxb.builder.UnmarshallerBuilder;
import by.prognoz.exjaxb.exception.DetailedJAXBException;
import by.prognoz.exjaxb.exception.ErrorCodes;
import by.prognoz.exjaxb.fs.FileSystemUtils;
import by.prognoz.exjaxb.namespace.NamespaceDecorator;
...
public class ExJAXB<T> {
   ...
   * Расширенная обработка исключительных ситуаций. Обобщен тип получаемого
   * результата.
   * 
   * @param srcXml                исходный документ одной строкой
   * @param srcXsd                xsd-схема для валидации (можно null)
   * @param <T>                   тип целевого класса
   * @param srcBean               объект целевого класса (.class)
   * @param isValidationEnabled   разрешить валидацию
  ...
  public <T> T unmarshall(String srcXml, Schema srcXsd, Class<T> srcBean, boolean isValidationEnabled, boolean isSavingTempEnabled)
   ...
   * 
   * @param <T>               тип целевого класса
   * @param targetObj         бин (сущность целевого класса)
   *     
   * @return                  org.w3c.dom.Document
   * @throws JAXBException    ошибки
   */
  ...
  /**
   * Выполняет автоматическую сериализацию объекта, задаваемого через targetObj
   * Имя рутового тега rootName
   * 
   * @param <T>
   * @param m
   * @param targetObj
   * @param rootName
   * @param doc             создание документа без создания фабрики для вывода результатов маршаллера
   * 
   * @return
   * @throws JAXBException
   */
  public <T> org.w3c.dom.Document marshall(Marshaller m, T targetObj, String rootName, String prefix, Document doc) throws JAXBException {
